...
import { getContentById } from "@/app/actions"
...
export async function POST(req: Request) {
  try {
    const { query, contentId } = await req.json()

    // Get content from server-side storage
    const content = await getContentById(contentId)

    if (!content) {
      return NextResponse.json({
        role: "assistant",
        content: "I'm sorry, I couldn't find the content you're referring to. The session may have expired.",
        id: Date.now().toString(),
      })
    }

    const { transcription, translation, language } = content
